...
call "!NPM_CMD!" install --no-audit --no-fund >nul 2>&1
if %errorlevel% neq 0 (
    call :err "npm install failed" "Could not install dependencies."
    goto :end
)
...
call "!NPM_CMD!" run build:exe >nul 2>&1
if %errorlevel% neq 0 (
    call :err "pkg compilation failed" "Could not compile executable."
    goto :end
)
...
docker build -f Dockerfile.build -t scrollscape-builder . >nul 2>&1
if %errorlevel% neq 0 (
    echo.
...
docker run --rm -v "%cd%\dist:/out" scrollscape-builder >nul 2>&1
if %errorlevel% neq 0 (
    echo.
    call :err "pkg compilation failed" "Docker build encountered an error."
    goto :end
)
goto :verify
